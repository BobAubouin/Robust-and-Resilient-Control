...
for i = 1:N-1
   ...
   obs = X(:,i);
   if i==1
       [u_mais,mpc_data,xn] = MPC_Robuste(MPCobj,obs,Ref,pred,U(:,1),mpc_data,0);
   else
       [u_mais,mpc_data,xn] = MPC_Robuste(MPCobj,obs,Ref,pred,U(:,i-1),mpc_data,0);
   end
...
%% test appartenance des perturbations


L=[];
flag=1;
for i=1:size(dX,2)
    int = [VD.Data(:,:,i)-Vm.Data(:,:,i)];
    L(i) = in(Zd1,int);
    if L(i)==0 && flag 
        disp(i);
        flag=0;
