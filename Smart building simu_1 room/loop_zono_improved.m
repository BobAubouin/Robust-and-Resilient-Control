...
%     end
   ...
   t = T(i);
   y = C*X(:,i);
   [dy,BIY,Replay_data,pred] = attacker_Y(y,Vm.Data(:,:,i:end) - Vmd2(:,1),t,BIY,Replay_data,choix_scenario,Data,Ts); %Vm.Data(:,:,i:end)
   dY(:,i+1) = dy;
   obs = X(:,i);
   if i==1
       [u_mais,mpc_data,xn,x_pred] = MPC_ROBUSTE_improved(MPCobj,obs,Ref,pred,U(:,1),mpc_data,1);
   else
       [u_mais,mpc_data,xn,x_pred] = MPC_ROBUSTE_improved(MPCobj,obs,Ref,pred,U(:,i-1),mpc_data,1);
   end
   ...
   X_k1 = A*X(:,i) + Bu*dU(:,i) + Bd*(VD.Data(:,:,i+1).*[1;1;1]-0*[0;Vm.Data(:,:,i+1)]);
   y = C*X_k1;
   X(:,i+1) = X_k1;
   Y(:,i+1) = y;
   disp(i)
